...
def generator(z, dim=64, reuse=True, training=True):
    ...
    with tf.variable_scope('generator', reuse=reuse):
        ...
        y = fc_bn_relu(y, 7 * 7 * dim * 2)
        y = tf.reshape(y, [-1, 7, 7, dim * 2])
        y = tf.image.resize_bilinear(y, [14,14])
        y = conv_bn_relu(y, dim, 3, 1)
        y = tf.image.resize_bilinear(y, [28,28])
        img = tf.tanh(tf.layers.conv2d(y, filters=1, kernel_size=3, padding='SAME',
