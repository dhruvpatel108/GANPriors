...
test_sample = np.reshape(test_data[digit_idx[it_no]], [28,28,1])
#np.random.seed(1008)
dim_like = 28*28*1
...
with tf.Session(graph=g) as sess:
    ...
    for n in range(n_iter):
        ...
        mask = np.ones((batch_size,28,28,1), dtype=bool)
    ...
    save_dir = './sample_test/monte_carlo/test/mask/mnist_wgan_gp{}_digit{}'.format(N, it_no)    
    utils.mkdir(save_dir+'/')
    ...
    cb = plt.imshow(x_map[0,:,:,0], cmap='viridis')
    plt.colorbar(cb, ax=axs)
    ...
    fig = plt.figure()
    axs = fig.add_subplot(111)
    cb = plt.imshow(x_mean[:,:,0], cmap='viridis')
    plt.colorbar(cb, ax=axs)
    ...
    fig = plt.figure()
    axs = fig.add_subplot(111)
    cb = plt.imshow(var[:,:,0], cmap='viridis')
    plt.colorbar(cb, ax=axs)
